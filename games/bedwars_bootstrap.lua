...
local GuiLibrary
local VWFunctions
local baseDirectory = (shared.VapePrivate and "vapeprivate/" or "newvape/")
...
local function resolveVoidwarePath(scripturl)
	...
	local mapped = {
		...
		["assetsversion.txt"] = "newvape/assetsversion.txt",
	}
	return mapped[scripturl] or ("newvape/"..scripturl)
end
...
for i,v in pairs({baseDirectory:gsub("/", ""), "newvape", "newvape/libraries", "newvape/games", "newvape/profiles", baseDirectory.."profiles", "newvape/assets", "newvape/CheatEngine"}) do
	if not isfolder(v) then makefolder(v) end
end
task.spawn(function()
	local success, assetver = pcall(function() return vapeGithubRequest("assetsversion.txt") end)
	if not isfile("newvape/assetsversion.txt") then writefile("newvape/assetsversion.txt", "0") end
	if success and assetver > readfile("newvape/assetsversion.txt") then
		redownloadedAssets = true
		if isfolder("newvape/assets") and not shared.VapeDeveloper then
			if delfolder then
				delfolder("newvape/assets")
				makefolder("newvape/assets")
			end
		end
		writefile("newvape/assetsversion.txt", assetver)
	end
end)
...
local GUIColor1 = {Hue = 0.93, Sat = 0.82, Value = 0.58}
local GUIColor2 = {Hue = 0.73, Sat = 0.88, Value = 0.62}
...
local Rewrite_Windows_Corresponder = {["Funny"] = "Blatant",["Hot"] = "Blatant",["Exploits"] = "Blatant",["Customisation"] = "Utility",["TP"] = "World",["Voidware"] = "Utility",["VoidwareDev"] = "Blatant",["GameScripts"] = "World"}
...
local function loadVape()
	...
	if not shared.VapeIndependent then
		...
		pcall(function() pload("VWUniversal.lua") end)
		...
		pload(fileName1)
		pload(fileName2)
		pcall(function() pload("games/dex_module.lua") end)
	...
	pcall(function()
		local src
		if isfile('newvape/guis/custom_theme.lua') then
			src = readfile('newvape/guis/custom_theme.lua')
		else
			src = shared.vapeGithubRequest('guis/custom_theme.lua')
		end
		if src and src ~= '' then
			local theme = loadstring(src, 'custom_theme')()
			if theme and theme.apply then
				shared.NovaTheme = theme.apply(GuiLibrary, {Brand = 'NOVA', Subtitle = 'BEDWARS'})
			end
		end
	end)
	enableRainbowUI()
